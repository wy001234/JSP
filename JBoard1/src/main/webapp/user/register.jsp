...
<script>
	...
	$(function(){
		...
		$('input[name = pass2]').focusout(function(){
			...
			if(pass1 == pass2){
				...
				if(regPass.test(pass2)){
					...
					isPassOK = false;
					$('.resultPass').css('color','red').text('비밀번호가 영문, 숫자, 특수문자 조합 최소 5자 이상');	
				}
				
			}else{
				isPassOK = false;
				$('.resultPass').css('color','red').text('비밀번호가 일치 하지않습니다.');
			}
		...
		//폼 전송이 시작될때 실행되는 폼 이벤트(전송버튼을 클릭햇을 때)
		$('.register > form').submit(function(){
            ...
            <main id="user" class="register">
                ...
                <form action="/JBoard1/user/proc/registerProc.jsp">
                    ...
                    <div>
                        <a href="/JBoard1/user/login.html" class="btnCancel">취소</a>
